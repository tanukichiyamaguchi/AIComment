...
- **2026-05-26**: `jissen_default` プロファイルに `master_sheet_name` を追加（全下書き宛先空の事故修正）。本番実行で Gmail 下書きが全件 TO 空になる事象を調査。ログに `Sheets: 参加者マスター 0件を取得 (参加者マスター)` が大量出力、原因は `jissen_default.yaml` が `master_sheet_name` 未指定で `config.MASTER_SHEET_NAME`（デフォルト `"参加者マスター"`）にフォールバック → `_ensure_master_sheet` が空タブを **自動作成** → 0 件読み込み → 全件宛先空でフォールバック下書き作成。実運用のタブ名は `テスト5_参加者マスター` だった。修正: `profiles/jissen_default.yaml` に `master_sheet_name: "テスト5_参加者マスター"` を明示追加 + 同じ事故を防ぐためのコメント（「省略するとデフォルトタブを自動作成→空のまま使用してしまい全下書き宛先空になる」）を残す。Q2/Q3/Q4 プロファイルは将来運用開始時にそれぞれの実タブ名を明示する運用とする。教訓: 「auto-create on missing」は安全側に見えて、実際には設定漏れを **黙って吸収して空動作する** という最悪のサイレント失敗を生む（fail-soft が fail-silent になっていた）。今後 `_ensure_*` 系で「シートが既存タブと一致しなかった」場合は INFO ではなく WARNING で「新規作成しました。設定漏れの可能性を確認してください」とログを出す改善余地あり（次回改修候補、本コミットには含めない）。
