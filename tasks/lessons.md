...
### P-030: 「再実行するだけで続きから動く」を唯一の永続ストアに根拠づけよ（ephemeral 実行環境の復旧設計）
GHA ランナーは ephemeral で、ジョブが kill されるとローカルの状態ファイル（`logs/batch_id.txt` 等）は次の実行に引き継がれない。Batch 投入後に step3/step4 で死んだ場合、再実行の step1 は「Sheets に何も記録されていない」ため全件を**再投入**し、Anthropic への課金が倍増していた（監査で発覚）。復旧手段が「アーティファクトから batch_id を掘り出して手動で `--step results`」しかない設計は、深夜・非エンジニア運用では実質「止まる」のと同じ。**Rule**: ephemeral 環境で長時間ジョブを走らせるシステムは、(1) 課金・副作用が確定するイベント（バッチ投入）を、システムの**唯一の永続ストア**（このプロジェクトでは Google Sheets）に投入直後へ記録し、(2) 実行開始時に「未回収イベント」を検知して**同じコマンドの再実行だけで**回収から自動再開できるようにする。状態は append-only の遷移ログ（投入済み → 回収完了/期限切れ）にすると競合・部分書き込みに強い。記録自体は fail-soft（記録失敗でランを止めない）、回収の完了記録も fail-soft（失敗しても次回再回収 → 出力側の冪等ガードが重複を防ぐ）と、各層の失敗が安全側に倒れるかを 1 つずつ確認すること。

### P-031: 重複判定キーは「その成果物自身」を指せ（親のキーを借りると孤児が恒久ロストする）
添付資料の重複判定を「メイン PDF の管理番号が処理済みか」で行っていたため、(1) メイン処理後に添付だけ後から Drive に追加、(2) メイン記録後・添付コピー前のクラッシュ再実行、のどちらでも添付資料が**恒久にスキップ**され続けた（警告ログは出るが誰も気づかない）。親子関係のある成果物で子の dedup キーに親のキーを使うと、「親だけ完了・子は未完了」という中間状態が「全部完了」と区別できなくなる。**Rule**: 出力の重複判定キーは、その成果物自身の記録（このケースでは出力一覧シートの `【添付資料】<元名>` マーカー）を指すこと。また子の処理が「同一ラン内の親の処理結果」に依存している場合は、親なしでも子を処理できるフォールバック経路（ここでは参加者マスターだけで出力先を解決）を用意し、依存を「最適化」に格下げする。

### P-032: 「item が results に無い」は 1 つの原因ではない — 分岐前に全事象を列挙せよ
`step4_generate_pdfs` は `custom_id not in results` を単一の「missing」原因として扱い、無条件で WARNING + `stats["missing"]` に計上していた。しかし回収実行（`--step results --batch-id` / 自動レジューム）では items が `reconstruct_items_from_drive`（Drive 全メイン PDF の再走査）で作られるため、results に無い item には「今回のバッチに含まれない過去処理済み PDF」「そもそも投入されなかった PDF（管理番号抽出不可等）」「本当に投入されたのに結果が返らなかった PDF」の 3 種類が混在する。単一原因決め打ちの分岐は、回収実行という「入力集合が変わる経路」を通したときに大量の誤報を生んだ（完了マーカー「未取得 N件」が実態と乖離）。**Rule**: 「〜が無い／不在」を検知する分岐を書くとき、その分岐に到達し得る**全ての経路**（正常系だけでなく回収・再構築・fallback 経路も含む）を洗い出し、原因ごとに扱いを分けること。「無い」の意味は呼び出し文脈（どこから来た item か）によって変わり得るので、判定に必要な情報（ここでは「過去処理済みか」「管理番号があるか」）は分岐の**前**に揃えてから多分岐で振り分ける。1 種類の原因のためのログ・カウンタを、性質の異なる 2 種類目の原因にも使い回さない（per-item ログの要否も原因ごとに異なる: 大量発生し得る「過去処理済み」は無言スキップ、稀な「本当の異常」だけ warning）。

### P-033: 共有予算を複数フェーズに分配するときは、フェーズ数ぶん独立に満額を渡さない
`run()` は Batch 結果のポーリング予算 `poll_max_seconds`（GHA 6h に収まるよう既定 5h）を、未回収バッチの自動レジューム（P-030）フェーズと通常の結果取得フェーズの両方に**独立に満額**渡していた。片方だけが走るケースを想定した設計のまま、両方が同一ラン内で直列に走るケース（未回収バッチがあり、かつ新規バッチも投入する）を見落としており、合計待機時間が上位の時間制約（GHA ジョブ timeout）を超えてジョブ kill され得た。**Rule**: 複数フェーズが同じ上位制約（ここでは GHA 6h）を共有するとき、各フェーズに制約の値をコピーして渡すのではなく、呼び出し元が単一の `deadline`（絶対時刻）を 1 つだけ持ち、各フェーズには「その時点で残っている予算」を都度計算して渡す。予算が尽きた後続フェーズは最小フロア値で「安全に諦める」ようにし（0 を渡すと 1 度も試行されずに即失敗するため）、上位の復旧機構（今回は次回実行の自動レジューム）に委ねる。

### P-035: 「記録は誰の分だけ」という仕様確認は、確定後も本番投入まで暫定として扱え
Phase 24 でチーム配布機能を実装する際、「出力一覧シートへの記録は報告者の 1 行のみでよいか」を
明示的に確認しユーザーは "従来どおり報告者のみ" と回答した。しかし本番投入後、Drive フォルダは
全員分正しく生成されたのに「シートを見ると提出者しか分からず不便」と気づき、記録範囲を全員分に
変更してほしいという追加要望が来た。ユーザーの初回回答自体は誤りではなく、**実物を見るまで
気づけなかった要望**だった。**Rule**: 「副作用（フォルダ／ファイル生成）はメンバー全員に広げるが
記録（シート／ログ）は代表者 1 行のみに絞る」という設計は、実データを見た後に「見えないと困る」
気づきが起きやすい典型パターン。可能なら実装時に「記録を絞ると "誰に配ったか" が事後に一覧できなく
なるがよいか」まで踏み込んで確認するか、既存の重複除去パターン（`passthrough_attachment` の
`【添付資料】<元名>` マーカー = 親と同じ管理番号を共有しつつ別行として記録する）を最初から
候補として提示する。今回は同じマーカー方式（`【チーム配布】<実践事例名>`）で対応し、
`get_processed_management_numbers` の重複判定（A 列の値の有無のみ見る）に影響を与えずに
メンバー行を追加できた——**新しい行を追記するだけの変更は、既存の「どのキーで重複判定するか」を
変えない限り安全に増やせる**、という設計余地を覚えておくとよい。

### P-034: リソース解放の finally は「取得直後」から全域を包め（部分カバーの try/finally は無いのと同じ経路がある）
`session_outputs_dir = mkdtemp(...)` に対する `finally: shutil.rmtree(...)` が、関数末尾のドラフト作成呼び出し**だけ**を包んでいた。取得（mkdtemp）と try 開始の間に数百行の処理（メインループ・添付ループ・disk 由来 JSON のパース）があり、その区間の未捕捉例外では rmtree に到達せず一時ディレクトリがリークしていた（GHA は ephemeral で無害だが、Codespaces/ローカルの反復実行で蓄積）。「finally があるから掃除は保証されている」という見た目が、カバー範囲の穴を隠す。**Rule**: リソース（一時ファイル/ディレクトリ・ロック・接続）の解放を finally で保証するときは、**取得の直後**に try を開始し、そのリソースを使う全区間を try 内に収める。途中に「per-item では捕捉されない例外源」（disk 読み込み・フィールドアクセス・外部 API のスナップショット取得）が無いかを、取得〜finally 間の行を列挙して確認する。可能なら with 文（コンテキストマネージャ）で構造的に強制するのが最善だが、既存の大きな関数に後付けする場合は「mkdtemp 直後に try:」の機械的インデントでも同じ保証が得られる。

## Session Log
- **2026-07-06**: Phase 24 — チーム事例の全員配布。ユーザー要望「チーム実践事例・チームMTG実践レポートはチーム全員のフォルダに保存したいが、ファイル名から所属チームを判別できない」→ 参加者マスターに F 列「所属チーム」を追加し、ファイル名冒頭の管理番号 ↔ A 列で報告者行を特定 → F 列の同値行全員へ配布する設計（ユーザー確定: マーカーは `チーム実践_`/`チームMTG_` 部分一致、シート記録は報告者 1 行のみ、Gmail は報告者のみ）。実装: `MasterRecord.team` は**末尾 default 付きフィールド**で追加（既存テストは全てキーワード構築＝無修正互換）、既存 5 列タブは `_ensure_sheet_with_header` がヘッダー非空なら書き換えない仕様のためそのまま共存。配布ヘルパー `distribute_team_copies` は (1) 報告者自身を除外（直前に既存フローがアップロード済み）、(2) **メンバーへの失敗は fail-soft にせず raise 伝播**（per-item error → シート未記録 → 再実行で配布やり直し。fail-soft だと「一部メンバー欠落のままシート記録済み＝恒久欠落」で P-031 の再演になる）、(3) 呼び出し位置は **`append_output_record` より前**（配布 → 記録の順序が復旧可能性の要）。e2e テストは `append_output_record` の side_effect で「記録時点で upload が 3 回済んでいる」を probe する方式で順序契約を機械検証（mock_calls の事後比較より頑健）。チーム名比較は normalize_name_for_match（P-009）。pytest 745 → 767 件 / mypy clean。
- **2026-07-06**: Phase 23 PR-4+PR-5 — 承認済み計画の最終 2 項目（PR #62 マージ後、小粒 2 件のため独立コミットで 1 PR に同梱）。(PR-4) workflow_dispatch の step choice に prepare/submit/pdfs を追加し、batch-orchestrator の 4 分割運用（GHA 6h を跨ぐ 1000 件実行）を UI から実行可能に（logs/ アーティファクトの手動復元が前提の上級操作と description に明記）。poll_max_minutes 入力（既定 300）を新設し --poll-max-minutes に配線（Batch モードのみ、src.main には無い引数のため分岐）。step=all の既存経路は完全不変。(PR-5) `utils.mask_name`（先頭 1 文字 + ＊、mask_email と同じ思想）を新設し、氏名・医院名を平文出力する全ログを grep 棚卸しでマスク: comment_generator 抽出/Batch 結果ログ、main・batch_main 完了ログ、run_common（医院名解決・下書き蓄積・25MB 分割・添付コピー・マスター解決）、gmail_client 下書き作成ログ、sheets_client（出力一覧追記・個人名 lookup 系 6 箇所）。**境界の判断**: マスクはログ専用。シート・出力ファイル名・Drive フォルダ名は業務データそのものなので平文のまま（伏せると成果物が壊れる）。Drive のフォルダ操作ログ（`医院フォルダ新規作成 (001_山田歯科)` 等）も P-019 の重複調査・運用デバッグに必須のため対象外とし、docstring に明記。既存テスト 2 件（fallback 警告に生医院名を期待）をマスク期待値 + `assertNotIn(生名)` に更新。pytest 740 件 pass / mypy clean。
- **2026-07-06**: Phase 23 PR-2 — 処理速度 3 件（PR #61 マージ後、計画の最終実装フェーズ）。(2a) step1 並列ダウンロード: ループ本体を `_prepare_one_target`（status タグ付き結果を返すワーカー、共有状態に触れない）+ `_consume_prepare_outcome`（メインスレッドの集計）に分離し、workers>1 で `ThreadPoolExecutor` + **`executor.map`（投入順 yield）** により items の順序を targets 順で決定論化（`plan_batch_chunks` のチャンク境界が再走でも安定）。既定 `STEP1_DOWNLOAD_WORKERS=1` はコードパスごと従来の逐次実行を温存し、本番 GHA だけ yml の env で 4 並列（退路 = env を 1 に戻すだけ）。RefreshError は fail-fast 伝播 + `shutdown(cancel_futures=True)`。(2b) Drive/Sheets サービスの thread-local キャッシュ: `build()`+認証+初回トークンリフレッシュ（数百ms×従来は呼び出しごと、1000件で2000回超）をプロセス/スレッドごと 1 回に。`get_*_service()` 自体は不変でテストのパッチ先を維持し、モジュール内の直呼び箇所だけ `_cached_*_service()` に置換。conftest の autouse に reset を追加（テスト間の mock サービスリーク防止）。**置換スクリプトの落とし穴**: 挿入したキャッシュ関数自身の内部の `get_sheets_service()` 呼び出しまで一括置換が書き換えて無限再帰になった（挿入→置換の順序で自分も対象になる）。一括置換は「挿入後のファイルに置換対象が何個あるべきか」を assert してから流すこと。(2c) `read_master_records` のプロセス内メモ化（key=(spreadsheet_id, sheet_name)）で step1 strict ガード + step4 の二重読みを解消。空結果も**同一オブジェクト**でキャッシュ（`assertIs` の同一性契約。別リテラルを返すと「キャッシュした」ことにならない）。pytest 725 → 740 件 / mypy clean。
- **2026-07-06**: Phase 23 PR-6 — 長時間ハードニング 3 件（PR #60 マージ後）。(6a) 一時ディレクトリ掃除の全域化: main.run / batch step4 の `finally: rmtree` がドラフト作成呼び出しだけを包んでおり、mkdtemp〜添付ループの未ガード区間（disk 由来 JSON パース・meta フィールドアクセス・get_recorded_attachment_names の Sheets 例外等）の例外でリークしていた。mkdtemp 直後からの try/finally に全域化（アンカーベースの機械変換で ~200 行をインデント、成功時挙動は完全同一、P-034）。(6b) 防御的パース: 回収経路で disk から読む `batch_results.json` の旧形式/手編集で `meta["comment"]` 等が KeyError → 未ガード区間でラン全体が死んでいたのを `.get` + 空 comment の per-item エラー化に。壊れた `batch_attachments.json` は生 Traceback ではなく「削除するか --step prepare を再実行」の復旧手順つき RuntimeError に（P-001）。(6c) ドラフト OFF 時のディスク逐次解放: `ENABLE_GMAIL_DRAFTS=false` でも全結合 PDF を最後まで保持していたのを、アップロード成功直後にサブディレクトリ即削除（ON は従来通り集約添付まで保持、回帰テストで固定）。**テストの落とし穴**: `src.batch_main.tempfile.mkdtemp` を無条件 return_value でパッチすると、ループ内の `tempfile.TemporaryDirectory`（内部で mkdtemp を呼ぶ）まで同じパスを返し、その exit がセッションディレクトリを削除してしまう → prefix で選別して本物に委譲する selective fake が必要だった。事前検証で「問題なし」確定: ログローテ（100MB×5世代・多重ハンドラガード）/ logs/ 全ファイル上書き型 / キャッシュ有界 / メモリ（1000件で~115MB）。pytest 719 → 725 件 / mypy clean。
- **2026-07-04**: Phase 23 PR-3 — 出力品質 2 件（PR #59 マージ後、承認済み計画の続き）。(3a) コメント品質ガード: `_parse_extraction` に keyword-only の `context` 引数を追加し（既存呼び出しは位置1引数で後方互換）、固有名詞スクラブ前後の文字数を計測。短縮時は INFO（`raw→scrubbed 文字数 + context`）、scrub 後 0<len<100 字は WARNING で可視化（P-001）。**warning-only の設計判断**: Batch で失敗化すると failed_ids → シート未記録 → 次ラン再投入 → 恒久再投入ループ（毎回課金）のリスクがあり、塞ぐには失敗管理番号の永続化が必要になるため見送り（docstring に明記）。「短いコメントは failed_ids に入れない」契約テストを追加。(3b) PDF レイアウト: じっせん君画像のジオメトリ（35mm角・下端+10mm）をモジュール定数に昇格し、本文描画下限を `box_y+40mm` 固定値から `画像帯上端+5mm クリアランス` の導出値（=50mm 相当）に変更。旧実装は満幅の最下行（baseline 65mm）が画像帯（45〜70mm... 正しくは 10〜45mm 帯の上端 70mm 換算）に食い込み得た。pdfplumber で全文字の y0 > 画像帯上端を機械検証する end-to-end テストを追加（描画可能行数 24→23 行、350字+段落は 15 行で収容力維持を回帰テストで担保）。pytest 712 → 719 件 / mypy clean。
- **2026-07-02**: Phase 23 PR-1 — 正確性バグ 3 件を修正（速度×品質改善計画の第1弾、コアのみ・Gmail/コスト系はスコープ外）。6レンズ監査(Explore)+実装設計(Plan)で確度検証済みの計画から実装。(1a) 回収実行（`--step results --batch-id` / 自動レジューム）で items が `reconstruct_items_from_drive`（Drive 全メイン PDF 再走査）から作られるため、過去処理済み PDF が results に無いだけで「コメント未取得」WARNING + missing 計上される誤報を修正。ループ先頭で mgmt_num を先に抽出し、results 無し時を「処理済み(無言skip)/管理番号抽出不可(info)/真のmissing(warning)」の3分岐に整理（P-032）。(1b) `_resume_open_batches` と `_collect_results_for_batches` が各々 poll_max_seconds を満額（既定5h）消費すると合計で GHA 6h を超えてジョブ kill され得た問題を、run() が単一 deadline を持ち `_remaining_seconds()` で残量を都度計算する方式に修正（P-033、フロア60秒）。(1c) `get_open_batch_ids` の重複除去が list+`not in`（線形探索）で `_バッチ管理` タブ肥大時に O(n²) だったのを挿入順保持 dict に置換（挙動不変）。pytest 704 → 712 件（+8）、mypy clean。既存テストは無傷（missing/skip_already_processed の既存契約・CB-3 添付ロスト防止経路は全て保全）。P-032 / P-033 を追加。
- **2026-07-02**: Phase 22 — 大量PDF運用の耐障害監査と修復（エージェントチーム監査）。ユーザー要件「1000 件規模の本番運用で止まらないこと」。6 レンズ（Batch スケール限界 / Google quota / クラッシュ復旧 / 長時間ラン / 直近リグレッション / リソース枯渇）の並列監査エージェント + 敵対的検証で 25 候補 → コード精読で確認できた問題を修復。**修復内容**: (A) 新規 0 件で `batch_id.txt` FileNotFoundError → 赤ラン（増分運用の定常ケースがクラッシュ）を正常終了に。(B) `submit_batch` の 256MB/バッチ上限ガード無し → `plan_batch_chunks`（純関数、ensure_ascii エスケープ後サイズで見積り）で自動分割・複数バッチ送信・`batch_id.txt` は 1 行 1 ID・`--batch-id` カンマ区切り対応。(C) pdf_text 10 万文字上限（コンテキスト超過 → 毎ラン再投入・再失敗ループの防止）。(D) バッチ状態を Sheets `_バッチ管理` タブに永続化し、`step=all` 再実行が未回収バッチを自動回収（再投入・二重課金防止、P-030）。(E) Gmail `create_draft` に指数バックオフ 5 試行 + `num_retries`。(F) 添付合計 17MB 超の下書きを自動分割（Gmail 25MB 上限の恒久失敗防止）。(G) 添付資料の dedup をマーカーベース + マスター単独ルーティングに（恒久ロスト防止、P-031）。(H) 通常モードに時間バジェット（既定 320 分、GHA 6h kill による成果物ロスト防止）。(I) `_ensure_sheet_with_header` のプロセス内キャッシュ（1000 行で read 2000 回超 → read quota 429 ストームの防止）。(J) Drive フォルダ解決のラン内キャッシュ（数千回の冗長全走査を削減）。(K) Sheets throttle 50→28/分（2 ラン同時でも合算 56 < 60）。(L) RefreshError の fail-fast + step1 全件失敗の loud 停止。(M) step3 ポーリング既定 5h（GHA 6h と整合、タイムアウト時は自動レジュームに引き継ぎ）。(N) artifact 名に run_attempt（re-run 時の 409 防止）。(R) マスタータブ名寄せの NFKC 正規化。(S) pdf_creator 切り捨て時の loud 警告。pytest 633 → 704 件 / mypy clean。**教訓**: 監査は「複数レンズの独立エージェント + 収束（同じ問題に複数レンズが到達）を確信度シグナルに使う」のが有効だった。P-030 / P-031 を追加。
- **2026-06-26**: 参加者マスタータブの名寄せ（フォルダ名がセミナー名を含めば自動対応）。ユーザー要望: 入力フォルダを年度・期ごとに分けつつ (例 `新人育成塾_2026_Q1`)、参加者マスターはセミナー単位 1 枚 (`参加者マスター(新人育成塾)`) で運用したい。実装: ``sheets_client.list_master_sheet_tabs`` で ``参加者マスター(...)`` 形式の既存タブを列挙し、``discover.resolve_master_sheet_name`` (純関数) で部分一致 + 最長一致でマッチング (例: フォルダ ``新人育成塾_2026_Q1`` × タブ ``参加者マスター(新人育成塾)`` → 後者を参照)。マッチなしは ``参加者マスター(<folder>)`` を fallback として返し、後段の HARD FAIL (P-028 / ``master_sheet_strict=True``) で運用者に通知。``DiscoveredContext`` に ``master_sheet_name`` フィールドを追加して ``resolve_context`` 内で確定させ、``RunConfig.from_discovered`` はそれを使う設計に再構成。Sheets API の metadata 取得失敗は致命扱いせず fallback に倒す（``try/except`` で WARNING のみ、HARD FAIL は後段が引き受ける）。pytest 617 → 633 件 (resolve_master_sheet_name 8 / resolve_context 結合 4 / list_master_sheet_tabs 4)、mypy clean。設計判断: (1) 純関数 ``resolve_master_sheet_name`` と API 呼び出し ``list_master_sheet_tabs`` を分離してテスト容易性確保、(2) 同長マッチは max() で決定論的（再走でも同じ結果）、(3) 不正形式タブ (``参加者マスター`` 括弧なし、``参加者マスター()`` 空セミナー) は無視して誤マッチを防ぐ。
...
- **2026-07-14**: チーム配布メンバーも出力一覧シートに記録（P-035）。Phase 24 本番投入後、ユーザーから「出力フォルダにはチーム全員のフォルダが生成されたが、出力一覧シートには提出者しかリスト化されない」と報告。Phase 24 実装時にユーザー自身が「出力一覧シートは報告者 1 行のみでよい」と明示的に確認・回答していたが、実データを見て初めて「配布先が一覧できないと不便」と気づいた形。対応: `run_common.distribute_team_copies` の戻り値を配布件数（`int`)から各メンバーの `{"clinic_name", "person_name", "drive_url"}` 辞書のリストに変更し、メンバー自身のアップロード結果（`webViewLink`）を呼び出し側に渡せるようにした。main.py / batch_main.py は配布成功後（P-031 の「配布 → 記録」順序は維持）、戻り値を使ってメンバーごとに `append_output_record` を追加で呼ぶ。メンバー行は報告者と**同じ管理番号**を使い、実践事例名を `【チーム配布】<実践事例名>` にして報告者本人の行と区別する（`passthrough_attachment` の `【添付資料】<元名>` マーカーと同じ「親と同じキーを共有しつつ別行として記録する」パターンを踏襲）。`get_processed_management_numbers` の重複判定（A 列の値の有無のみ見る）はメンバー行が増えても影響を受けない。既存 e2e テスト（`test_team_file_distributed_to_all_members_before_sheet_record` / `test_distribution_happens_before_sheet_record` / batch 側の対応テスト）を新しい `append_output_record` 呼び出し回数（3 回 = 報告者 1 + メンバー 2）に更新。pytest 767 件維持（既存テストの契約更新のみ、新規追加なし）、mypy clean。README §4-5 を更新。教訓は P-035 参照。reporter: ユーザー（本番運用後のフィードバック）。

- **2026-07-14**: CDC 医院見学レポート（`XXX-YY` 2 セグメント）が出力されない事象を修正。ユーザー報告「CDC の医院見学レポートは通常の `XXX-YY-Z` ではなく `XXX-YY` の表記で、現在出力されない」。根本原因は `utils._MANAGEMENT_NUMBER_PATTERN` が `^(\d{3,5})-\d{2}-\d`（3 セグメント固定）で、2 セグメントのファイル名では `extract_management_number` が空文字列 → `select_new_targets` で「管理番号なしスキップ」されていたこと。修正: パターンの 3 番目セグメントを任意化 `^(\d{3,5})-\d{2}(?:-\d)?`。`(?:-\d)?` は**貪欲（既定）**なので `001-01-0` は従来どおり `001-01-0` を返し（2 セグメントで止めない）、既存の 3 セグメント抽出は完全に不変。`001-01医院見学.pdf` は `001-01` を返す。CDC レポートは専用マーカー（`チーム実践_`/`【添付資料】`）を持たず純粋に管理番号の形だけで判別されるが、2 セグメント管理番号はマスターの `NNN-NN`（個人単位）と**完全一致**するため `lookup_participant_by_management_number` / `lookup_email_by_clinic_and_person` / `lookup_clinic_name` はいずれも無改修で機能する。重複判定（`get_processed_management_numbers`、A 列の値の有無のみ）も無改修。警告文言「NNN-NN-N 形式でない」→「NNN-NN-N / NNN-NN 形式でない」に更新（main/batch_main/run_common 計 4 箇所）。既存 test_utils の 18 ケース（3 セグメント・桁数境界・全角・非文字列など）を新パターンで**全て手検算してから**実装し、退行ゼロを確認。pytest 767 → 773 件（+6: CDC mgmt 5 / clinic 1、うち 1 件は 3 セグメント非退行の回帰）、mypy clean。教訓: (1) **正規表現で「末尾セグメントを任意化」するときは貪欲/非貪欲がデフォルト挙動を左右する**。非貪欲 `(?:-\d)??` にすると既存の 3 セグメントが 2 セグメントに切り詰められて全件退行する。任意化は「既存の最長一致を保つ = 貪欲」であることを既存テストの手検算で必ず固定する。(2) **フォーマット差だけで種別が決まる文書は、判別ロジックを 1 箇所（管理番号抽出）に集約すると波及が最小になる**。CDC レポートはマーカー不要で、抽出パターンを広げるだけで lookup/dedup/フォルダ振り分けが全て無改修で成立した（管理番号がマスターの個人単位キーと一致する設計のおかげ）。reporter: ユーザー（本番運用でのフィードバック）。

- **2026-08-25**: チーム配布メンバーの医院フォルダ URL がどのシートにも記録されない欠落を修正（Phase 27）。ユーザー報告（経営塾ベーシックのチーム宿題）は Phase 25 の再報告に見える文面（「スプレッドシートには提出者しかリスト化されない」「フォルダは作成されているが URL がリストに反映されない」）だったが、**GHA 実行ログの一次調査で切り分けた**: run #116(7/31)/#117(8/5)/#118(8/25) いずれも出力一覧シートへ【チーム配布】メンバー行 52 件が正常追記されており（Phase 25 は機能している）、一方 `_医院` シート（医院フォルダURL一覧）への追加は 8 件 = 提出者の医院のみ。配布で「Drive: 医院フォルダ新規作成」が約 25 医院分走っているのに、その URL の記録経路が存在しなかった。根本原因: `ClinicFolderRecorder.record` の呼び出しが報告者経路にしかなく、`distribute_team_copies` は メンバーの `clinic_folder_id` を返していなかった。修正: 戻り値メンバー辞書に `clinic_number` / `clinic_folder_id` を追加し、main/batch のメンバーループで `clinic_recorder.record` を呼ぶ（既存の医院番号デデュープで 1 医院 1 行を維持）。教訓: (1) **「前に直したはずの症状」の再報告は、同じ機能の別シート/別出力面の欠落を疑う**。ユーザーは「リスト」という言葉で出力一覧と _医院 シートを区別しない。修正済み機能の再報告を「動いていない」と決めつけず、本番ログで「どの出力面に何行書かれたか」を数えてから直す。(2) **新しい出力先（フォルダ等）を作る機能を追加したら、その URL/ID を記録する既存シートへの反映も同じフェーズで棚卸しする**。Phase 24 で医院フォルダを新規作成する経路を足したとき、_医院 シートへの記録は報告者経路にしか無いことを見落とした。副次修正: anthropic SDK を `>=0.40.0,<1.0` に上限ピン。1.0.0 は `messages.create()` から `temperature` 引数を削除（httpx2 移行を伴うメジャー）しており、通常モード（src.main）の同期呼び出しが TypeError で即死し得た + mypy が typeddict-unknown-key で落ちる。requirements が `>=` のみだと**本番は成功していても（Batch 経路は TypedDict が実行時検証しないため無事）休眠経路が静かに壊れる**。メジャーバージョン境界は上限ピンで守り、移行は専用フェーズで行う。教訓: (3) **可変ピン（>= のみ）の依存は、fresh install する CI/本番でメジャーが出た日に突然壊れる**。休眠コードパス（今回は非バッチ同期呼び出し）は本番成功ログでは検出できないため、SDK メジャーは即日ピンして専用フェーズで移行する。reporter: ユーザー（本番運用フィードバック）+ ログ一次調査。

- **2026-08-25 (2)**: Phase 27 の PR 報告直後、ユーザーから認識合わせの指摘（スプレッドシートのスクリーンショット付き）を受け、真の要望が「シートへの URL 記録」ではなく「**出力フォルダを医院軸に加えてチーム軸（`チーム別/<所属チーム>/`）でも振り分ける**」ことだと判明 → Phase 28 として実装。仕様の分岐点（対象範囲 / フォルダ内構成 / 設置場所 / URL 一覧シートの要否）は AskUserQuestion で 4 点確定してから着手した（チーム宿題のみ / PDF 直置き / 「チーム別」親フォルダ配下 / `_チーム` タブあり）。実装: `distribute_to_team_folder`（報告者の F 列でチーム解決 → `チーム別/<チーム名>/` find_or_create → `upload_pdf` 同名スキップ冪等、失敗 raise = P-031、未登録/チーム空は None で警告は直前の `distribute_team_copies` に委譲）+ `TeamFolderRecorder`（`_チーム` タブへ 1 チーム 1 行、キーは `normalize_name_for_match` で揺れ吸収）+ sheets_client の 2 列シート 3 部品（既存の `_医院` 実装をミラー）。pytest 778 → 794 / mypy clean。教訓: (1) **ユーザーの「リスト/フォルダ」系の要望は、出力面（シートか Drive か・どの軸か）の認識合わせを最初にやる**。Phase 25（出力一覧）→ Phase 27（_医院 シート）→ Phase 28（チーム軸フォルダ）と、同じ「チーム全員分が見えない」という不満の解釈が 3 回変わった。スクリーンショットや実データを見せてもらう + 選択肢を提示して確定させるのが最短だった。(2) **新しい出力軸を足すときは、既存軸のパターン（find_or_create → upload 冪等 → Recorder でシート記録 → デデュープ）をそのままミラーする**と、テストもドキュメントも既存の型に沿って最小差分で書ける。reporter: ユーザー（スクリーンショット + AskUserQuestion 回答）。
