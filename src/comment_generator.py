...
【comment フィールドの改行ルール】
- 文脈が切り替わるところ（話題の転換、称賛から提案への移行など）で改行文字 `\\n` を挿入し、段落として分けてください。
- 機械的に句点「。」ごとに改行するのではなく、読み手にとって自然な区切りで段落を分けてください（おおむね 1〜2 文ごとに 1 段落が目安）。
- 改行は文と文のあいだだけで使い、1 文の途中には入れないでください。段落間の空行は不要（`\\n\\n` ではなく `\\n` 1 つ）。"""


# ── プロンプトはじっせん実践事例（SYSTEM_PROMPT）に統一 ──
# ファイル名のテーマ（読書 / LIGレポート / パートナー / チームMTG 等）による
# プロンプト振り分けは廃止し、全件 ``SYSTEM_PROMPT`` で生成する（ユーザー方針）。
