...
# PDF のファイル名先頭に埋め込まれた管理番号のパターン。
# ``<3〜5桁>-<2桁>[-<1桁>]``（先頭セグメント = 医院番号は 3〜5 桁可変、
# 2 番目のセグメントは 2 桁固定、3 番目のセグメント ``-<1桁>`` は任意）。
# 先頭セグメントの ``(...)`` は ``extract_clinic_number`` が医院番号として再利用する。
#
# 3 番目のセグメントを任意にしている理由:
#   - 実践事例 PDF は提出単位の ``NNN-NN-N``（3 セグメント）。
#   - CDC の医院見学レポートは ``NNN-NN``（2 セグメント、3 番目が無い）。
# ``(?:-\d)?`` は貪欲（既定）なので、``001-01-0`` のように 3 番目がある場合は
# 従来どおり ``-0`` まで取り込む（2 セグメントだけを取って ``001-01`` で
# 止まることはない）。3 番目が無い ``001-01医院見学.pdf`` は ``001-01`` を返す。
...
def extract_management_number(filename: str) -> str:
    ...
    ファイル名の先頭に埋め込まれた管理番号を抽出する。2 つの形式に対応する:

    - 実践事例 PDF: ``NNN-NN-N``（3 セグメント。先頭 3〜5 桁 - 数字2 - 数字1）
      例: ``001-01-0実践事例.pdf`` → ``001-01-0``、``00123-45-6.pdf`` → ``00123-45-6``。
    - CDC 医院見学レポート: ``NNN-NN``（2 セグメント。3 番目のセグメントが無い）
      例: ``001-01医院見学レポート.pdf`` → ``001-01``。

    3 番目のセグメントは任意。ある場合は必ず取り込む（``001-01-0`` を
    ``001-01`` で止めない）ため、既存の 3 セグメント抽出は不変。

    Args:
        filename: PDF のファイル名

    Returns:
        抽出した管理番号（2 セグメントなら 6〜8 文字、3 セグメントなら 7〜9 文字）。
...
def extract_clinic_number(filename: str) -> str:
    ...
    管理番号 ``NNN-NN-N``（実践事例）/ ``NNN-NN``（CDC 医院見学レポート）の
    先頭セグメント（3〜5桁）が医院番号。
    例: ``001-01-0実践事例.pdf`` → ``001``、``00123-45-6.pdf`` → ``00123``、
    ``001-01医院見学レポート.pdf`` → ``001``。
