...
### 4-4. メールアドレス一覧シート（Gmail 下書き作成用ルックアップ表）

実践事例 PDF の処理が完了するたびに、その人物宛の **Gmail 下書き**（コメント付き PDF を添付）が自動作成されます。下書きの送信先（TO / CC）は、専用シート **`メールアドレス一覧`** から `(医院番号, 個人名)` で引きます。

- シート名は **`メールアドレス一覧`**（既定）。シートが無ければ初回実行時に **自動作成され、ヘッダー行だけ書き込まれた空シート**が用意されます。ユーザーは後から行を追記してください。
- 列は **5 列**:

  | 列 | ヘッダー | 内容 |
  |----|---------|------|
  | A | 医院番号 | 管理番号 `NNN-NN-N` の先頭セグメント（3〜5桁、例 `001`） |
  | B | 医院名 | 参照用（システムは見ない。ユーザーが行を判別するために使う） |
  | C | 個人名 | コメント対象の個人氏名（AI 抽出値と完全一致する必要あり） |
  | D | 個人メール | その個人のメールアドレス。空欄なら医院メールへフォールバック |
  | E | 医院メール | 医院共通のメールアドレス。同じ医院の複数行で**同じ値を重複記入してよい**（ユーザー側の利便性のため） |

- 1 行 per (医院, 個人)。医院に複数の個人がいれば複数行になります。
- 形式不正のメール（`@` がない等）はその列だけ空扱い + 警告ログで通知し、もう片方の列は使用されます。

#### ルックアップ・フォールバック挙動

PDF 処理の管理番号から医院番号と個人名を取り出し、メールアドレス一覧シートを以下の優先順位で引きます:

1. `(医院番号, 個人名)` で完全一致行を探す。見つかった行で:
   - **個人メールあり** → TO = 個人メール、CC = 医院メール（医院メール空なら CC なし）
   - **個人メール空、医院メールあり** → TO = 医院メール、CC なし
   - **両方空** → 次の手順へフォールバック
2. 完全一致行がない、または上記で両方空だった場合、同じ医院番号の **別の行** から医院メールを 1 つ拾う → TO = 医院メール、CC なし
3. すべて空（その医院番号が一切登録されていない） → **下書き作成をスキップ + 警告ログ**

メール未登録時のスキップは PDF 処理自体には影響しません（PDF アップロード・出力一覧シート追記は通常通り完了）。下書きを後から作りたい場合は、メールアドレス一覧シートに行を追加 → 出力一覧シートの該当行を削除 → 再実行、の手順でその管理番号が「未処理」扱いに戻り、再処理と一緒に下書きも再作成されます。
...
- 医院番号（フォルダ名の接頭辞）は管理番号から機械抽出した安定値、医院名は AI 抽出値です。
...
4. メールアドレス一覧シートを 1 回読み込み（無ければ自動作成・空のヘッダーのみ）
5. 処理対象（新規）の各メインPDFについて：
   ...
   f. 出力一覧シートに 1 行追記
   g. メールアドレス一覧から (医院番号, 個人名) を引き、Gmail 下書きを作成
      （メール未登録なら警告スキップ、例外時も処理を続行）
   h. 管理番号 → (医院名, 個人名) の対応表に登録
6. 添付資料パススルー（AI 処理なし）— メインPDF処理の完了後：
   a. 添付資料の管理番号で 5-h の対応表を引く
...
│   ├── sheets_client.py      # Google Sheets API（出力一覧シート追記 + メールアドレス一覧読み取り）
