...
- **医院フォルダは医院番号で識別されます。** AI が同じ医院でも医院名を違う表記で抽出した場合（例: `三浦歯科医院` / `三浦歯科` / `医療法人三浦歯科`）、同じ医院番号のフォルダが既にあれば **そのフォルダを再利用** します（医院名部分はリネームしません＝最初に作られた名前を正とする）。フォルダ識別子部分（`<医院番号>_`）のみで前方一致判定するため、語そのものが違う医院名表記揺れでも重複フォルダが作られません。
...
- **同じ医院番号で複数フォルダが既存する場合**（本仕様導入前にできた重複）は、フォルダ ID 昇順で決定論的に 1 つを再利用し、警告ログに重複 ID 一覧を出します（自動統合・自動リネームは行いません＝破壊的変更を避けるため。手動統合を推奨）。
