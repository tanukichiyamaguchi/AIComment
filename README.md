...
- **target_folder モード**（推奨・後述 §「フォルダ自動検出モード」）: タブ名は **`参加者マスター(<フォルダ名>)`** をセミナーごとに使います（例: 入力フォルダ `新人育成塾` → タブ `参加者マスター(新人育成塾)`）。タブが**不在 or 0 件のときは PDF 処理に入る前に HARD FAIL で即停止**します（下書きが全件宛先空・医院名フォルダが AI 抽出値で作成される事故を未然に防ぐ）。Batch モードでは Anthropic API 投入前に Step 1 でガードします。
...
- **参加者マスタータブ名 = `参加者マスター(<フォルダ名>)`**（例: `新人育成塾` → タブ `参加者マスター(新人育成塾)`）。セミナーごとに独立した参加者管理を行う
