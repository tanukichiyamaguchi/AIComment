...
- 列は **3 列**:
  ...
  | B | 医院名 | AI が PDF から抽出した医院名（医院番号は付かない） |
...
- 医院番号（フォルダ名の接頭辞）は管理番号から機械抽出した安定値、医院名は AI 抽出値です。フォルダの検索・再利用は表記揺れを吸収する正規化マッチングに委ねます（同じ医院でも医院名の半角/全角・空白の揺れで重複フォルダを作らない）。
- 管理番号が `NNN-NN-N` 形式で始まらない PDF は医院番号を持てないため、医院番号なし（医院名のみ）のフォルダ名にフォールバックします。
...
4. 処理対象（新規）の各メインPDFについて：
   ...
   f. 出力一覧シートに 1 行追記
   g. 管理番号 → (医院名, 個人名) の対応表に登録
5. 添付資料パススルー（AI 処理なし）— メインPDF処理の完了後：
   a. 添付資料の管理番号で 4-g の対応表を引く
   b. 対応するメインがこの実行に無い → スキップ（警告ログ）
   c. 元PDFのバイト列をそのまま、メインと同じ <医院名>/<個人名>/ フォルダへ
      元ファイル名のままアップロード
